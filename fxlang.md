...
While this design makes the battle engine incredibly dynamic, it also makes the battle engine very inefficient when running events. Do we really need to recalculate a Mon's effective ability for _every_ event? Given that effects are not changing between every event, the answer is "no," and this is where **effect caching** comes into play.
