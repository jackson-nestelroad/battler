  ...
  if (connection.status === "connecting") {
    ...
    if (secondsRemaining !== null) {
      if (secondsRemaining > 0) {
        const attemptStr = retryCount !== null ? ` (Attempt ${retryCount})` : "";
        connectionMessage = `${baseWord} in ${secondsRemaining}s...${attemptStr}`;
      } else {
        connectionMessage = `${baseWord} now...`;
      }
    } else {
      connectionMessage = `${baseWord}...`;
    }
  }
