import { BattleState, UiLogEntry, alterBattleState, newBattleState } from "battler-state";
...
describe("LogFormatter", () => {
  ...
  it("should format complex Move log", () => {
    ...
    const log = result!.messages[0];
    expect(log.category).toBe(LogCategory.Primary);
    expect(log.context.__CAPITALIZED_MON).toEqual({
      text: "The opposing Pikachu",
      monRef: { Active: { player: "p2", position: 0, name: "Pikachu", side: 0 } },
    });
  ...
  it("should handle battle type disambiguation for critical hits", () => {
    ...
    const singlesResult = formatter.format(
      entry as unknown as UiLogEntry,
      singlesState as unknown as BattleState,
    );
    const doublesResult = formatter.format(
      entry as unknown as UiLogEntry,
      doublesState as unknown as BattleState,
    );
    const multiResult = formatter.format(
      entry as unknown as UiLogEntry,
      multiState as unknown as BattleState,
    );
    const multiLowercaseResult = formatter.format(
      entry as unknown as UiLogEntry,
      multiStateLowercase as unknown as BattleState,
    );
  ...
  it("should format boost magnitudes correctly", () => {
    ...
    const getBoostLog = (by: number, max?: boolean) => {
      ...
      const result = formatter.format(
        entry as unknown as UiLogEntry,
        mockState as unknown as BattleState,
      );
  ...
  it("should format plain damage with empty messages and damage with from as Secondary", () => {
    ...
    expect(stringifyLog(srResult!.messages[0])).toBe(
      "Pointed stones dug into the opposing Charizard!",
    );
  ...
  it("should format Damage and Heal notices with non-possessive mon and percentage format", () => {
    ...
    expect(stringifyLog(healResult!.messages[0])).toBe(
      "The opposing Squirtle had its HP restored!",
    );
  });
  ...
  it("should emit two messages and item notice for Mega Evolution", () => {
    ...
    expect(stringifyLog(result!.messages[0])).toBe(
      "Venusaur's Venusaurite is reacting to your Mega Bracelet!",
    );
  ...
  it("should format Primal Reversion", () => {
    ...
    expect(result!.messages.length).toBe(1);
    expect(stringifyLog(result!.messages[0])).toBe(
      "Kyogre's Primal Reversion! It reverted to its primal state!",
    );
  });
  ...
  it("should format debug logs correctly with Secondary category", () => {
    ...
    expect(stringifyLog(result!.messages[0])).toBe(
      "DEBUG: ModifyDamage: Unexpected state connector",
    );
  });
  ...
  it("should format Battle Bond differently for trainer vs wild Mon", () => {
    ...
    const trainerSelfResult = formatter.format(
      trainerSelfEntry as UiLogEntry,
      wildState as unknown as BattleState,
    );
    expect(trainerSelfResult).not.toBeNull();
    expect(trainerSelfResult!.messages[0].key).toBe("activate__ability_battlebond");
    expect(stringifyLog(trainerSelfResult!.messages[0])).toBe(
    ...
    const wildResult = formatter.format(
      wildEntry as UiLogEntry,
      wildState as unknown as BattleState,
    );
    expect(wildResult).not.toBeNull();
    expect(wildResult!.messages[0].key).toBe("activate__ability_battlebond__wild");
    expect(stringifyLog(wildResult!.messages[0])).toBe(
  ...
  it("should format Disguise damage with ability and damage notices", () => {
    ...
    const state = {
      field: {
        sides: [{ players: { p1: { name: "Alice" } } }, { players: { p2: { name: "Bob" } } }],
      },
    };
  ...
  it("should format both item and ability notices for Booster Energy activating Protosynthesis", () => {
    ...
    expect(stringifyLog(result!.messages[0])).toBe(
      "Walking Wake used its Booster Energy to activate Protosynthesis!",
    );
  });
  ...
  it("should format both source item notice and target damage notice simultaneously on contact item damage", () => {
    ...
    expect(stringifyLog(result!.messages[0])).toBe(
      "Zapdos was hurt by the opposing Ferrothorn's Rocky Helmet!",
    );
  });
  ...
  it("should format Skill Swap flow with ability notices on each swap log entry", () => {
    ...
    expect(plusleResult!.notices.length).toBe(2);
    ...
    expect(plusleResult!.notices[1]).toEqual({
      type: "Ability",
      name: "Drizzle",
      mon: "Plusle's",
    ...
    expect(minunResult!.notices.length).toBe(2);
    ...
    expect(minunResult!.notices[1]).toEqual({
      type: "Ability",
      name: "Soundproof",
      mon: "The opposing Minun's",
      monRef: { Active: { side: 1, position: 0, name: "Minun", player: "p2" } },
  ...
  it("should attribute main ability to both source and target in abilitystart with source", () => {
    const testLogs = [
      "side|id:0|name:Side 1",
      "side|id:1|name:Side 2",
      "player|id:ash|name:Ash|side:0|position:0",
      "player|id:ai-random-1|name:AI|side:1|position:0",
      "switch|player:ash|position:1|name:Grumpig|health:100/100|species:Grumpig|level:50|gender:M",
      "switch|player:ai-random-1|position:1|name:Alomomola|health:100/100|species:Alomomola|level:50|gender:F",
      "battlestart",
      "turn|turn:1",
      "abilityend|mon:Grumpig,ash,1|ability:Trace|from:ability:Trace",
      "abilitystart|mon:Grumpig,ash,1|ability:Refrigerate|source:Alomomola,ai-random-1,1|from:ability:Trace",
    ];
    const state = alterBattleState(newBattleState(), testLogs);
    const lastTurnLogs = state.ui_log[state.ui_log.length - 1];
    const formatter = new LogFormatter({ localPlayerId: "ash" });

    const abilityEndFormatted = formatter.format(lastTurnLogs[1], state);
    expect(abilityEndFormatted?.notices).toEqual([
      {
        type: "Ability",
        name: "Trace",
        mon: "Grumpig's",
    ...
    const abilityStartFormatted = formatter.format(lastTurnLogs[2], state);
    expect(abilityStartFormatted?.notices).toEqual([
      {
        type: "Ability",
        name: "Refrigerate",
        mon: "The opposing Alomomola's",
        monRef: {
          Active: {
            side: 1,
            position: 0,
    ...
    expect(stringifyLog(abilityStartFormatted!.messages[0])).toBe(
      "Grumpig traced the opposing Alomomola's Refrigerate!",
    );
  });
  ...
  describe("Extension logs", () => {
    ...
    it("should resolve standard log hints from hints.logs as Hint category when logs template is null", () => {
      i18next.addResource(
        "en",
        "translation",
        "hints.logs.ability__ability_gooey",
        "Contact with {{MON}} lowered the attacker's Speed!",
      );
  ...
  describe("MON_NAME context variable usage", () => {
    ...
    it("should format mega evolution logs using MON_POSSESSIVE and MON", () => {
      ...
      expect(stringifyLog(megaRes!.messages[0])).toBe(
        "The opposing Charizard's Charizardite X is reacting to ai-random-1's Mega Bracelet!",
      );
      expect(stringifyLog(megaRes!.messages[1])).toBe(
        "The opposing Charizard has Mega Evolved into Mega Charizard X!",
      );
      ...
      expect(stringifyLog(dragonRes!.messages[0])).toBe(
        "ai-random-1's fervent wish has reached the opposing Rayquaza!",
      );
      expect(stringifyLog(dragonRes!.messages[1])).toBe(
        "The opposing Rayquaza has Mega Evolved into Mega Rayquaza!",
      );
    });
    ...
    it("should format structural log entries (turn, continue, time, residual)", () => {
      ...
      expect(formatter.formatEntry(turnEntry as UiLogEntry)).toEqual([{ kind: "turn", turn: "3" }]);
      expect(formatUiLogEntry(turnEntry as UiLogEntry)).toEqual([{ kind: "turn", turn: "3" }]);
  ...
  describe("Spectator Mode", () => {
    ...
    it("should format ability, item, and damage notices with player ownership for spectators", () => {
      ...
      const abilityResult = spectatorFormatter.formatEntry(
        abilityEntry as UiLogEntry,
        singlesState,
      );
  ...
  describe("ability, item, source, from, and of notice variants", () => {
    it("should format abilitystart without source (only target gets notice)", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "abilitystart",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Cinderace", player: "p1" } },
          ability: "Libero",
        },
      };
      const result = formatter.format(entry as UiLogEntry);
      expect(result?.notices).toEqual([
        {
          ...
          name: "Libero",
          mon: "Cinderace's",
          monRef: { Active: { side: 0, position: 0, name: "Cinderace", player: "p1" } },
        },
      ]);
    });

    it("should format abilitystart with source (both target and source get notice)", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "abilitystart",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Smeargle", player: "p1" } },
          ability: "Intimidate",
          source: { Active: { side: 1, position: 0, name: "Gyarados", player: "p2" } },
          from: "move:Role Play",
        },
      };
      const result = formatter.format(entry as UiLogEntry);
      expect(result?.notices).toEqual([
        ...
        },
        {
          ...
          name: "Intimidate",
          mon: "Smeargle's",
          monRef: { Active: { side: 0, position: 0, name: "Smeargle", player: "p1" } },
        },
      ]);
    });

    it("should format abilityend with only target notice", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "abilityend",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Slaking", player: "p1" } },
          ability: "Truant",
        },
      };
      const result = formatter.format(entry as UiLogEntry);
      expect(result?.notices).toEqual([
        {
          ...
          name: "Truant",
          mon: "Slaking's",
          monRef: { Active: { side: 0, position: 0, name: "Slaking", player: "p1" } },
        },
      ]);
    });

    it("should format abilityend with from and of (target gets ability, of gets from)", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "abilityend",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Slaking", player: "p1" } },
          ability: "Truant",
          from: "ability:Neutralizing Gas",
          of: { Active: { side: 1, position: 0, name: "Weezing", player: "p2" } },
        },
      };
      const result = formatter.format(entry as UiLogEntry);
      expect(result?.notices).toEqual([
        {
          ...
          mon: "The opposing Weezing's",
          monRef: { Active: { side: 1, position: 0, name: "Weezing", player: "p2" } },
        },
        {
          type: "Ability",
          name: "Truant",
    ...
    it("should format itemstart without source (only target gets item notice)", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "itemstart",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Snorlax", player: "p1" } },
          item: "Leftovers",
        },
      };
      const result = formatter.format(entry as UiLogEntry);
      expect(result?.notices).toEqual([
        {
          ...
          name: "Leftovers",
          mon: "Snorlax's",
          monRef: { Active: { side: 0, position: 0, name: "Snorlax", player: "p1" } },
        },
      ]);
    });

    it("should format itemstart with source (both target and source get item notice)", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "itemstart",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Aipom", player: "p1" } },
          item: "Choice Band",
          source: { Active: { side: 1, position: 0, name: "Tyranitar", player: "p2" } },
          from: "move:Thief",
        },
      };
      const result = formatter.format(entry as UiLogEntry);
      expect(result?.notices).toEqual([
        {
          type: "Item",
          name: "Choice Band",
          mon: "The opposing Tyranitar's",
          monRef: { Active: { side: 1, position: 0, name: "Tyranitar", player: "p2" } },
        },
        {
          type: "Item",
          name: "Choice Band",
          mon: "Aipom's",
          monRef: { Active: { side: 0, position: 0, name: "Aipom", player: "p1" } },
        },
      ]);
    });

    it("should format itemend without source (only target gets item notice)", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "itemend",
        values: {
    ...
    it("should format itemend with source (e.g. Bug Bite / Pluck, only source whose item ended gets item notice)", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "itemend",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Scizor", player: "p1" } },
          item: "Sitrus Berry",
          source: { Active: { side: 1, position: 0, name: "Snorlax", player: "p2" } },
          eat: true,
        },
      };
      const result = formatter.format(entry as UiLogEntry);
      expect(result?.notices).toEqual([
        {
          ...
          monRef: { Active: { side: 1, position: 0, name: "Snorlax", player: "p2" } },
        },
      ]);
    });

    it("should attribute from with no of implicitly to target mon (never source)", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "damage",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Charizard", player: "p1" } },
          from: "item:Life Orb",
          damage: [10n, 100n],
        },
      };
      ...
      expect(result?.notices).toContainEqual({
        ...
        name: "Life Orb",
        mon: "Charizard's",
        monRef: { Active: { side: 0, position: 0, name: "Charizard", player: "p1" } },
      });
    });

    it("should attribute from with of to the of mon", () => {
      const formatter = new LogFormatter({ localPlayerId: "p1" });
      const entry: Partial<UiLogEntry> = {
        title: "damage",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Lucario", player: "p1" } },
          from: "ability:Rough Skin",
          of: { Active: { side: 1, position: 0, name: "Garchomp", player: "p2" } },
          damage: [12n, 100n],
        },
    ...
    it("should keep distinct notices for mons with identical formatted names in mirror matches", () => {
      // Observer perspective where both player-1 and player-2 resolve to 'The opposing Ralts\'s'
      const formatter = new LogFormatter();
      const entry: Partial<UiLogEntry> = {
        title: "abilitystart",
        values: {
          mon: { Active: { side: 0, position: 0, name: "Ralts", player: "player-1" } },
          ability: "Insomnia",
          source: { Active: { side: 1, position: 0, name: "Ralts", player: "player-2" } },
          from: "ability:Trace",
        },
      };
      const result = formatter.format(entry as UiLogEntry);
      ...
      expect(result?.notices).toEqual([
        {
          type: "Ability",
          name: "Insomnia",
          mon: "player-2's Ralts's",
          monRef: { Active: { side: 1, position: 0, name: "Ralts", player: "player-2" } },
        },
        {
          type: "Ability",
          name: "Insomnia",
          mon: "player-1's Ralts's",
          monRef: { Active: { side: 0, position: 0, name: "Ralts", player: "player-1" } },
        },
      ]);
    });
  });
