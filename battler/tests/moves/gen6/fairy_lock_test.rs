...
fn fairy_lock_traps_all_mons() {
    ...
    assert_matches::assert_matches!(battle.set_player_choice("player-1", "switch 1"), Err(err) => {
        assert_eq!(format!("{err:#}"), "invalid choice 0: cannot switch: Sylveon is trapped")
    });
    assert_matches::assert_matches!(battle.set_player_choice("player-2", "switch 1"), Err(err) => {
        assert_eq!(format!("{err:#}"), "invalid choice 0: cannot switch: Sylveon is trapped")
    });
...
#[test]
fn ghost_types_resist_fairy_lock() {
    let mut battle = make_battle(0, team().unwrap(), team().unwrap()).unwrap();
    assert_matches::assert_matches!(battle.start(), Ok(()));
