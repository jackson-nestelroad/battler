...
fn skitty() -> Result<TeamData, Error> {
    serde_json::from_str(
        r#"{
            "members": [
                {
...
fn fake_out_only_works_on_first_turn() {
    ...
    let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
        r#"[
            "move|mon:Skitty,player-2,1|name:Fake Out|target:Skitty,player-1,1",
            "split|side:0",
            "damage|mon:Skitty,player-1,1|health:83/110",
            "damage|mon:Skitty,player-1,1|health:76/100",
            "cant|mon:Skitty,player-1,1|reason:Flinch",
            "residual",
            "turn|turn:2",
            ["time"],
