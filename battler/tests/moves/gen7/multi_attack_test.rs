...
#[test]
fn multi_attack_is_normal_type_by_default() {
    let mut battle = make_battle(0, silvally().unwrap(), kecleon().unwrap()).unwrap();
    ...
    let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
        r#"[
            ...
            "split|side:1",
            "damage|mon:Kecleon,player-2,1|health:17/120",
            "damage|mon:Kecleon,player-2,1|health:15/100",
            "residual",
            "turn|turn:2"
        ]"#,
    )
...
#[test]
fn multi_attack_changes_type_from_memory() {
    let mut team = silvally().unwrap();
    team.members[0].item = Some("Bug Memory".to_owned());
