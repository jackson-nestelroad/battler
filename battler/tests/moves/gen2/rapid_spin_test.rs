...
mod rapid_spin_test {
    ...
    fn rapid_spin_clears_user_entry_hazards() {
        ...
        let mut battle =
            make_battle(&data, 0, forretress().unwrap(), forretress().unwrap()).unwrap();
        ...
        let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
            r#"[
                ...
                "sidestart|side:0|move:Spikes",
                "residual",
                "turn|turn:3",
                ["time"],
                "move|mon:Forretress,player-2,1|name:Toxic Spikes",
                "sidestart|side:0|move:Toxic Spikes",
                "residual",
                "turn|turn:4",
                ["time"],
                ...
                "damage|mon:Forretress,player-2,1|health:128/135",
                "damage|mon:Forretress,player-2,1|health:95/100",
                "end|mon:Forretress,player-1,1|move:Bind|from:move:Rapid Spin|of:Forretress,player-1,1",
                "end|mon:Forretress,player-1,1|move:Leech Seed|from:move:Rapid Spin|of:Forretress,player-1,1",
                "sideend|side:0|move:Spikes|from:move:Rapid Spin|of:Forretress,player-1,1",
                "sideend|side:0|move:Toxic Spikes|from:move:Rapid Spin|of:Forretress,player-1,1",
                "residual",
                "turn|turn:7"
            ]"#,
        )
