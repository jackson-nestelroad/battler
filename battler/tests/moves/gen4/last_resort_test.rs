...
fn last_resort_fails_if_only_move_known() {
    ...
    let mut battle = make_battle(0, team, munchlax().unwrap()).unwrap();
...
fn last_resort_only_usable_after_all_other_moves_used() {
    let mut battle = make_battle(0, munchlax().unwrap(), munchlax().unwrap()).unwrap();
    assert_matches::assert_matches!(battle.start(), Ok(()));
...
fn last_resort_fails_if_not_known() {
    ...
    let mut battle = make_battle(0, munchlax().unwrap(), team).unwrap();
    assert_matches::assert_matches!(battle.start(), Ok(()));
