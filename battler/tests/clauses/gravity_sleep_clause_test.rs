...
fn allows_full_accuracy_sleep_move() {
    ...
    assert_matches::assert_matches!(
        battle.update_team("player-1", full_accuracy_sleep_team().unwrap()),
        Ok(())
    );
}
...
fn disallows_low_accuracy_sleep_move() {
    let mut battle = make_battle(0).unwrap();
    assert_matches::assert_matches!(battle.update_team("player-1", low_accuracy_sleep_team().unwrap()), Err(err) => {
...
fn allows_low_accuracy_sleep_move_without_gravity() {
    let mut battle = make_battle(0).unwrap();
    assert_matches::assert_matches!(
        battle.update_team("player-1", low_accuracy_sleep_no_gravity_team().unwrap()),
        Ok(())
    );
}
...
fn disallows_low_accuracy_sleep_move_with_gmax_orbeetle() {
    let mut battle = make_battle(0).unwrap();
    assert_matches::assert_matches!(battle.update_team("player-1", low_accuracy_sleep_gmax_orbeetle_team().unwrap()), Err(err) => {
        assert_matches::assert_matches!(err.downcast_ref::<ValidationError>(), Some(err) => {
...
fn allows_low_accuracy_sleep_move_with_non_gmax_orbeetle() {
    ...
    let mut battle = make_battle(0).unwrap();
    assert_matches::assert_matches!(battle.update_team("player-1", team), Ok(()));
}
