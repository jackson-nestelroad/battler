...
mod snow_test {
    ...
    fn snow_lasts_for_five_turns() {
        ...
        let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
            r#"[
                ...
                "start",
                "switch|player:player-1|position:1|name:Dewgong|health:100/100|species:Dewgong|level:50|gender:M",
    ...
    fn snow_boosts_ice_defense() {
        let data = LocalDataStore::new_from_env("DATA_DIR").unwrap();
        let mut battle =
            make_battle(&data, 345332, dewgong().unwrap(), blastoise().unwrap()).unwrap();
        assert_eq!(battle.start(), Ok(()));
