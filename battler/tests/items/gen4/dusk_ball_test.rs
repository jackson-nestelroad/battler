...
fn dusk_ball_does_not_increase_catch_rate_in_day() {
    ...
    assert_matches::assert_matches!(
        battle.set_player_choice("protagonist", "item duskball"),
        Ok(())
    );
    ...
    let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
        r#"[
            "useitem|player:protagonist|name:Dusk Ball|target:Graveler,wild,1",
            "catchfailed|player:protagonist|mon:Graveler,wild,1|item:Dusk Ball|shakes:1",
            "residual",
            "turn|turn:2"
        ]"#,
    )
...
fn dusk_ball_increases_catch_rate_at_night() {
    ...
    let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
        r#"[
            "useitem|player:protagonist|name:Dusk Ball|target:Graveler,wild,1",
            "catchfailed|player:protagonist|mon:Graveler,wild,1|item:Dusk Ball|shakes:3",
            "residual",
            "turn|turn:2"
        ]"#,
    )
