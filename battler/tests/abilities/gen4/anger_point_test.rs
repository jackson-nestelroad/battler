...
fn anger_point_maximizes_attack_on_crit() {
    ...
    let mut battle = make_battle(&data, 0, primeape().unwrap(), primeape().unwrap()).unwrap();
    ...
    let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
        r#"[
            "move|mon:Primeape,player-1,1|name:Frost Breath|target:Primeape,player-2,1",
            "crit|mon:Primeape,player-2,1",
            "split|side:1",
