...
fn leaf_guard_prevents_status_in_sun() {
    ...
    let mut battle = make_battle(&data, 0, budew().unwrap(), budew().unwrap()).unwrap();
...
fn leaf_guard_prevents_previous_yawn_in_sun() {
    ...
    let mut battle = make_battle(&data, 0, budew().unwrap(), team).unwrap();
    assert_matches::assert_matches!(battle.start(), Ok(()));
    ...
    let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
        r#"[
            ...
            "ability|mon:Budew,player-1,1|ability:No Ability|from:move:Skill Swap|of:Budew,player-2,1",
