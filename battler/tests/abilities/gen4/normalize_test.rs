...
fn normalize_changes_move_type_to_normal() {
    ...
    let mut battle = make_battle(&data, 0, delcatty().unwrap(), delcatty().unwrap()).unwrap();
    ...
    let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
        r#"[
            "move|mon:Delcatty,player-1,1|name:Brick Break|target:Delcatty,player-2,1",
            "split|side:1",
            "damage|mon:Delcatty,player-2,1|health:72/130",
            "damage|mon:Delcatty,player-2,1|health:56/100",
            "residual",
            "turn|turn:2"
        ]"#,
    )
