...
fn rough_skin_damages_attacker_on_contact() {
    let mut battle = make_battle(0, sharpedo().unwrap(), sharpedo().unwrap()).unwrap();
...
fn protective_pads_protect_from_rough_skin() {
    ...
    let mut battle = make_battle(0, player, sharpedo().unwrap()).unwrap();
    assert_matches::assert_matches!(battle.start(), Ok(()));
