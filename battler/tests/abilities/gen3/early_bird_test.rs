...
fn early_bird_wakes_up_twice_as_fast() {
    ...
    let mut battle = make_battle(0, team, seedot().unwrap()).unwrap();
    ...
    let expected_logs = serde_json::from_str::<Vec<LogMatch>>(
        r#"[
            ...
            "move|mon:Seedot,player-1,1|name:Rest|noanim",
            "fail|mon:Seedot,player-1,1|what:heal",
            "fail|mon:Seedot,player-1,1",
            "cant|mon:Seedot,player-2,1|from:status:Sleep",
            "residual",
            "turn|turn:5",
            "continue",
            ...
            "move|mon:Seedot,player-2,1|name:Rest|noanim",
            "fail|mon:Seedot,player-2,1|what:heal",
            "fail|mon:Seedot,player-2,1",
            "residual",
            "turn|turn:6"
        ]"#,
    )
