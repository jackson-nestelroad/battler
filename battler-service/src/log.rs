...
impl Log {
    fn new(battle: Uuid, side: Option<usize>) -> Self {
    ...
    pub fn append<I, S>(&mut self, entries: I, global_logs: &mut Vec<GlobalLogEntry>)
    where
        ...
        S: Into<String>,
    {
        ...
        self.publish_from(last_index, global_logs);
    }

    fn publish_from(&self, index: usize, global_logs: &mut Vec<GlobalLogEntry>) {
        for (i, entry) in self.entries[index..].iter().enumerate() {
            ...
            global_logs.push(GlobalLogEntry {
                battle: self.battle,
                side: self.side,
                entry,
            });
...
impl SplitLogs {
    ...
    pub fn new(battle: Uuid, sides: usize) -> Self {
        Self {
            public_log: Log::new(battle, None),
            per_side_logs: Vec::from_iter((0..sides).map(|side| Log::new(battle, Some(side)))),
        }
    }
...
mod log_test {
    ...
    async fn filters_split_logs() {
        ...
        let mut logs = SplitLogs::new(Uuid::from_u64_pair(0, 128), 2);
        logs.append(
            [
                "time|time:123",
                "abc|def",
    ...
    async fn publishes_filtered_logs() {
        ...
        logs.append(
            ["split|side:0", "ghi|hp:255/255", "ghi|hp:100/100", "public"],
            &mut global_logs,
        );
