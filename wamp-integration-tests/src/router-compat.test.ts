...
const originalCreateTransport = (autobahn as any).Connection.prototype._create_transport;
(autobahn as any).Connection.prototype._create_transport = function() {
    const connectionInstance = this;
    const transport = originalCreateTransport.call(this);
    if (transport) {
        const originalSend = transport.send;
        transport.send = function(msg: any) {
            if (msg && msg[0] === 5) { // AUTHENTICATE
                const extra = msg[2] || {};
...
describe('WAMP Router Compatibility Tests', () => {
    ...
    test('T1.6: Registration/Subscription Cleanup on Abrupt Disconnect', async () => {
        ...
        await assert.rejects(
            caller.session!.call('com.compat.cleanup'),
    ...
    test('T1.8: Subscription Cleanup on Abrupt Disconnect', async () => {
        ...
        await subscriber.session!.subscribe('com.compat.cleanup_sub', () => { subCount++; });
        await otherSub.session!.subscribe('com.compat.cleanup_sub', () => { otherCount++; });
        ...
        const closed = new Promise<void>(r => {
            subscriber.onclose = () => { r(); return false; };
        });
        subscriber.close();
        await closed;
        await new Promise(r => setTimeout(r, 200));
        ...
        await new Promise(r => setTimeout(r, 200));
        assert.strictEqual(subCount, 1,
            'Disconnected subscriber must not receive further events');
        assert.strictEqual(otherCount, 2,
    ...
    test('T3.2: Pub/Sub with Acknowledgment', async () => {
        ...
        const pubResult = await client.session!.publish('com.compat.topic', [], {}, { acknowledge: true });
    ...
    test('T3.3: Prefix-based Subscription Matching', async () => {
        ...
        const events: any[] = [];
        await clientA.session!.subscribe('com.compat', (args, kwargs, details) => {
    ...
    test('T3.4: Wildcard-based Subscription Matching', async () => {
        ...
        const events: any[] = [];
        await clientA.session!.subscribe('com.compat..status', (args, kwargs, details) => {
            events.push({ args, topic: details.topic });
        }, { match: 'wildcard' });
        ...
        assert.strictEqual(events.length, 2);
        assert.deepStrictEqual(events.map(e => e.topic), ['com.compat.node1.status', 'com.compat.node2.status']);
    ...
    test('T3.6: Pattern-Based Pub/Sub (Prefix & Wildcard)', async () => {
        ...
        await subPrefix.session!.subscribe('com.compat.prefix', (args) => {
            prefixCount++;
        ...
        await subWildcard.session!.subscribe('com.compat.wildcard..event', (args) => {
            wildcardCount++;
    ...
    test('T3.7: Publisher Identification (Disclose Publisher)', async () => {
        ...
        await subscriber.session!.subscribe(
            ...
            (_args: any, _kwargs: any, details: any) => { disclosedDetails.push(details); }
        );
        ...
        await subscriber.session!.subscribe(
            ...
            (_args: any, _kwargs: any, details: any) => { hiddenDetails.push(details); }
        );
    ...
    test('T3.8: Pub/Sub exclude_me Option', async () => {
        ...
        await client.session!.subscribe('com.compat.excludeme', () => { selfCount++; });
        await otherSub.session!.subscribe('com.compat.excludeme', () => { otherCount++; });
        ...
        assert.strictEqual(otherCount, 1,
            'Other subscriber must receive the event');

        // Explicit exclude_me: false — publisher MUST receive its own event
        await client.session!.publish('com.compat.excludeme', [], {}, { acknowledge: true, exclude_me: false });
        await new Promise(r => setTimeout(r, 200));
        assert.strictEqual(selfCount, 1,
            'Publisher must receive own event when exclude_me: false');
        assert.strictEqual(otherCount, 2,
            'Other subscriber must still receive the event');
    ...
    test('T4.2: Calling Non-existent Procedure', async () => {
        const caller = await connectClient(router.port);
        await assert.rejects(
            caller.session!.call('com.compat.nonexistent'),
    ...
    test('T4.3: Unregistering a Procedure', async () => {
        ...
        await assert.rejects(
            caller.session!.call('com.compat.temp'),
            (err: any) => err.error === 'wamp.error.no_such_procedure'
        );
    ...
    test('T4.4: RPC Error Propagation', async () => {
        ...
        await callee.session!.register('com.compat.error', () => {
            throw new autobahn.Error('com.compat.custom_error', ['failed details'], { extra_key: 'extra_val' });
        });

        await assert.rejects(
            caller.session!.call('com.compat.error'),
            (err: any) => {
    ...
    test('T4.5: Shared Registrations (Invocation Policies)', async () => {
        ...
        await assert.rejects(
            callee2.session!.register('com.compat.shared.single', () => 2),
    ...
    test('T4.6: RPC Pattern Matching (Prefix & Wildcard)', async () => {
        ...
        // 1. Prefix Matching
        await callee.session!.register('com.compat.prefix', (args, kwargs, details) => {
        ...
        // 2. Wildcard Matching
        await callee.session!.register('com.compat.wildcard..proc', (args, kwargs, details) => {
            return details.procedure;
        }, { match: 'wildcard' });
        ...
        await assert.rejects(
            caller.session!.call('com.compat.wildcard.foo.other'),
            (err: any) => err.error === 'wamp.error.no_such_procedure'
        );

        // 3. Priority/Conflict Resolution: Exact vs Prefix
        await callee.session!.register('com.compat.match.exact', () => 'exact');
        await callee.session!.register('com.compat.match', () => 'prefix', { match: 'prefix' });
    ...
    test('T4.7: Caller Identification (Disclose Caller)', async () => {
        ...
        await callee.session!.register('com.compat.disclose.optin', (args, kwargs, details) => {
            return details.caller_authid !== undefined ? details.caller_authid : 'hidden';
        });

        // Call with disclose_me = true
        const resOptInTrue = await caller.session!.call('com.compat.disclose.optin', [], {}, { disclose_me: true });
        ...
        await callee.session!.register('com.compat.disclose.force', (args, kwargs, details) => {
            return details.caller_authid !== undefined ? details.caller_authid : 'hidden';
    ...
    test('T4.8: RPC Call Timeout', async () => {
        ...
        await assert.rejects(
            caller.session!.call('com.compat.slow', [], {}, { timeout: 300 }),
    ...
    test('T4.10: Progressive Call Results', async () => {
        ...
        const callPromise = caller.session!.call('com.compat.progressive', [], {}, {
