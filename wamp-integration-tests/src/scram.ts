...
export async function saltPassword(
  ...
  if (kdf === "pbkdf2") {
    const keylen = 32;
    const hash = crypto.pbkdf2Sync(cleanPassword, saltBytes, iterations, keylen, "sha256");
    const hashPHC = hash.toString("base64").replace(/=/g, "");
    return `$pbkdf2-sha256$i=${iterations}$${saltB64}$${hashPHC}`;
  } else if (kdf === "argon2id13" || kdf === "argon2id-13") {
    const m = memory ?? 4096;
    const rawHash = await argon2.hash(cleanPassword, {
    ...
    const hashPHC = rawHash.toString("base64").replace(/=/g, "");
    return `$argon2id$v=19$m=${m},t=${iterations},p=1$${saltB64}$${hashPHC}`;
  } else {
...
export interface ScramResponse {
  ...
  signature: string;
  /** Base64-encoded server signature, for verifying the WELCOME message. */
...
export async function generateScramResponse(
  ...
  const serverFirst = `r=${challenge.nonce},s=${challenge.salt},i=${challenge.iterations}`;
  // client_final_no_proof = c=,r=<challenge_nonce>  (empty channel binding for crossbar compatibility)
  const clientFinalNoProof = `c=,r=${challenge.nonce}`;
